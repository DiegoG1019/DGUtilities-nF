...
namespace System.Collections.NanoLinq
{
    ...
    public class ValueTypeArrayList : IList, ICollection, IEnumerable, ICloneable
    {
        internal class ValueTypeArrayListEnumerator : IEnumerator
        {
            ...
            public object Current => wrappedarray[currentPosition % wrappedarray.items.Length];

            public bool MoveNext() => currentPosition < lastPosition ? ++currentPosition < lastPosition : false;
        ...
        public int Add(object value) => InsertItemAtPosition(Count, value);

        public void Clear() => Array.Clear(items, 0, items.Length);

        public bool Contains(object value) => IndexOf(value) >= 0;

        public int IndexOf(object value) => Array.IndexOf(items, value, 0, itemCount);

        public void Insert(int index, object value) => InsertItemAtPosition(index, value);
        ...
        {
            get => index >= 0 && index < itemCount ? items.GetValue(index) : throw new ArgumentOutOfRangeException();
            set => items[index] = index >= 0 && index < itemCount ? value as ValueType : throw new ArgumentOutOfRangeException();
        }
        ...
        public IEnumerator GetEnumerator() => new ValueTypeArrayListEnumerator(this);
        ...
        public static implicit operator ValueType[](ValueTypeArrayList vtal) => (ValueType[])vtal.Clone();

        public static implicit operator ValueTypeArrayList(ValueType[] vals) => new ValueTypeArrayList(vals.Length, vals);
    }
}
